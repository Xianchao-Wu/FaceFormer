...
def init_biased_mask(n_head, max_seq_len, period): 
    # n_head=4, max_seq_len=600, period=25 for 'BIWI' ||| n_head=4, max_seq_len=600, period=30 for 'vocaset'
    ...
    bias = - torch.flip(bias,dims=[0]) # 从25个-23，到25个0 ||| 开始是30个'-19'，后面是30个‘0’.
    ...
    mask = (torch.triu(torch.ones(max_seq_len, 
        max_seq_len)) == 1).transpose(0, 1) # [600, 600], 左下角为True，对角线上都为True. 1个True, ..., 600个True
    mask = mask.float().masked_fill(mask == 0, 
            float('-inf')).masked_fill(mask == 1, float(0.0)) # 左下角为0，对角线为0，其他位置为-inf 
    mask = mask.unsqueeze(0) + alibi # mask is from [600, 600] to [1, 600, 600]; alibi=[4, 600, 600]
...
class Faceformer(nn.Module):
    def __init__(self, args):
        ...
        self.audio_encoder = Wav2Vec2Model.from_pretrained(
                "facebook/wav2vec2-base-960h") # TODO try other pretrained wav2vec models??? 
        # TODO 
        # wav2vec 2.0 weights initialization
        self.audio_encoder.feature_extractor._freeze_parameters()
        # 这是冻住一部分参数[NOTE 7层卷积]，4,200,448=4.2M; 剩下的92,215,197=92.2M 是需要训练的, vocaset
    ...
    def forward(self, audio, template, vertice, 
        ...
        frame_num = vertice.shape[1] # 帧数，[1, 160, 15069], frame_num=160, 即：160视频帧
        ...
        # self.dataset='vocaset', frame_num=160 
        ...
        if teacher_forcing: # False, not in NOTE
            ...
            for i in range(frame_num): # 160=帧数, 这是用的是参考答案frames num
                ...
                vertice_out = self.transformer_decoder(vertice_input, 
                        hidden_states, tgt_mask=tgt_mask, memory_mask=memory_mask)
                # 1. [1, 1, 64]; y[t-1] of frame tensor
                # 2. torch.Size([1, 160, 64]); audio tensor as memory
                # 3. [4, 1, 1]; frame causal masking 因果mask, 只能看左边
                # 4. [1, 160]                , batch seq length masking
                ...
                # Linear(in_features=15069, out_features=64, bias=True) 新预测出来的最新的一帧人脸
