...
class Dataset(data.Dataset):
    ...
    def __getitem__(self, index):
        ...
        print('audio={}, vertice={}, fn={}'.format(
            audio.shape, vertice.shape, file_name))
...
def read_data(args):
    ...
    # 'vocaset/templates.pkl'

    with open(template_file, 'rb') as fin:
        templates = pickle.load(fin,encoding='latin1') 
        # dict, keys=dict_keys(['FaceTalk_170904_00128_TA', 
    ...
    for r, ds, fs in os.walk(audio_path): # len(fs)=475
        for f in tqdm(fs):
            if f.endswith("wav"): # 'FaceTalk_170915_00223_TA_sentence21.wav'
                ...
                # 'vocaset/wav/FaceTalk_170915_00223_TA_sentence21.wav', sr=22000
                #import ipdb; ipdb.set_trace()
                speech_array, sampling_rate = librosa.load(wav_path, sr=16000) 
                print(wav_path, librosa.get_duration(filename=wav_path))

                # TODO 这里可以指定使用的audio的采样率; 
                # e.g., speech_array.shape=(69067,), 
                # 本来音频采样率是sr=22000，现在是按照16000读取的。

                input_values = np.squeeze(  
                        processor(speech_array,sampling_rate=16000).input_values)
                # 上面是对音频的预处理. output is input_values.shape=(69067,)
                key = f.replace("wav", "npy") 
                # 'FaceTalk_170915_00223_TA_sentence21.npy'
                ...
                # temp from (5023, 3) to (15069,) 一帧模板

                vertice_path = os.path.join(vertices_path,f.replace("wav", "npy")) 
                # 'vocaset/vertices_npy/FaceTalk_170915_00223_TA_sentence21.npy'
                ...
                if not os.path.exists(vertice_path):
                    ...
                    if args.dataset == "vocaset":
                        vertice_data = np.load(
                                vertice_path, allow_pickle=True) 
                        print(vertice_path, vertice_data.shape, 
                                vertice_data[::2, :].shape)
                        data[key]["vertice"] = vertice_data[::2, :] 
                        # 本来是259视频帧：(259, 15069) --> half 
                        # --> 现在是只要130视频帧了：(130, 15069)
    ...
    # 8, 'FaceTalk_170728_03272_TA FaceTalk_170904_00128_TA 
    # FaceTalk_170725_00137_TA FaceTalk_170915_00223_TA 
    # FaceTalk_170811_03274_TA FaceTalk_170913_03279_TA 
    # FaceTalk_170904_03276_TA FaceTalk_170912_03278_TA'

    subjects_dict["val"] = [i for i in args.val_subjects.split(" ")] 
    # 2, 'FaceTalk_170811_03275_TA FaceTalk_170908_03277_TA'

    subjects_dict["test"] = [i for i in args.test_subjects.split(" ")] 
    # 2, 'FaceTalk_170809_00138_TA FaceTalk_170731_00024_TA'

    splits = {
        'vocaset':{'train':range(1,41),
                'val':range(21,41),'test':range(21,41)}, 
            # TODO why in this range? 
    ...
    # train_data[0] is a dict with keys = 
    #     dict_keys(['audio', 'name', 'template', 'vertice'])
...
def get_dataloaders(args):
    ...
    return dataset 
    # {'train': <torch.utils.data.dataloader.DataLoader object at 0x7f8c67ddd6d0>, 
    # 'valid': <torch.utils.data.dataloader.DataLoader object at 0x7f8c73905df0>, 
    # 'test': <torch.utils.data.dataloader.DataLoader object at 0x7f8c7b8b3610>} 
    # for 'vocaset'
